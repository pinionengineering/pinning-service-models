...
// AssertPinStatusRequired checks if the required fields are not zero-ed
func AssertPinStatusRequired(obj PinStatus) error {
...
// AssertRecursePinStatusRequired recursively checks if required fields are not zero-ed in a nested slice.
// Accepts only nested slice of PinStatus (e.g. [][]PinStatus), otherwise ErrTypeAssertionError is thrown.
func AssertRecursePinStatusRequired(objSlice interface{}) error {
	return AssertRecurseInterfaceRequired(objSlice, func(obj interface{}) error {
		aPinStatus, ok := obj.(PinStatus)
		if !ok {
			return ErrTypeAssertionError
		}
		return AssertPinStatusRequired(aPinStatus)
	})
}
